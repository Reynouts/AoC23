...
def main():
    ...
    print(f'p1: {sum(reduce(lambda acc, c: ((ord(c) + acc) * 17) % 256, seq, 0) for seq in seqs)}')

    boxes = defaultdict(list)
    for seq in seqs:
        offset = -2 if seq[-1].isdigit() else -1
        label, op = seq[:offset], seq[offset:]
        box = reduce(lambda acc, c: ((ord(c) + acc) * 17) % 256, label, 0)
        boxes = process(box, label, op, boxes)
